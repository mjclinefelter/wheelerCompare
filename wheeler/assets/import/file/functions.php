...
function gtcdi_admin_menu() {
	add_submenu_page( 'edit.php?post_type=gtcd', 'AutoSales XML & CSV File Importer', 'Import Vehicles', 'administrator', 'gtcdi_importer', 'gtcdi_import_display_importer' );
}
			...
			foreach($value['tax'] as $tax_key => $tax_value):	
				...
				if($value['tax'][$tax_key.'_tier']):
					...
					foreach($tax_value as $key1=>$value1):
						...
						if($key1 == 'make'):
							$tax_key = 'makemodel';
							$term = term_exists( $value1, $tax_key, 0 );
						elseif($key1 == 'model'):
							$tax_key = 'makemodel';
							
							if(isset($value['tax']['make']) && !empty($value['tax']['make'])):
								// get make term id, if it exists
								$term = term_exists( $value['tax']['make'], 'makemodel', 0 );
								
								$parentId = 0;
								if(isset($term['term_id'])): $parentId = $term['term_id'];
								else:
									$term = wp_insert_term(
										$value['tax']['make'],
										'makemodel',
										array(
											'description'=> '',
											'slug' => strtolower(preg_replace('#[a-zA-Z0-9\-]+#', '', $value['tax']['make'])),
											'parent'=> 0
										)
									);
								...
								$term = term_exists( $value1, $tax_key, $parentId );
							endif;
						...
						elseif($key1 == 'city'):
							...
							if(isset($value['tax']['state']) && !empty($value['tax']['state'])):
								// get state term id, if it exists
								$term = term_exists( $value['tax']['state'], 'location', 0 );
								...
								if(isset($term['term_id'])): $parentId = $term['term_id'];
									...
									$term = wp_insert_term(
										...
										array(
											'description'=> '',
											'slug' => strtolower(preg_replace('#[a-zA-Z0-9\-]+#', '', $value['tax']['state'])),
											'parent'=> 0
										)
									);
					...
					if(is_array($tax_value)):
						foreach($tax_value as $key1=>$value1):
							...
							elseif($key1 == 'model'):
								...
								if(isset($value['tax']['make']) && !empty($value['tax']['make'])):
									...
									if(isset($term['term_id'])): $parent_id = $term['term_id'];
									else:
										$term = wp_insert_term(
											$value['tax']['make'],
											'makemodel',
									...
									$term = term_exists( $value1, $tax_key, $parent_id );
								endif;
							...
							if(!$term_id):
								...
								if(!is_wp_error($term)): array_push($term_id_array, (int) $term['term_id']); endif;
							else:
						...
						if($tax_key == 'make'):
							$tax_key = 'makemodel';
							$term = term_exists( $tax_value, $tax_key, 0 );
						elseif($tax_key == 'model'):
							$tax_key = 'makemodel';
							if(isset($value['tax']['make']) && !empty($value['tax']['make'])):
								// get make term id, if it exists
								$term = term_exists( $value['tax']['make'], 'makemodel', 0 );
								...
								$term = term_exists( $tax_value, $tax_key, $parent_id );
							endif;
						elseif($tax_key == 'state'):
							$tax_key = 'location';
							$term = term_exists( $tax_value, $tax_key, 0 );
						elseif($tax_key == 'city'):
							$tax_key = 'location';
							...
							$term = term_exists( $tax_value, $tax_key );
						endif;
						...
						if(!$term_id):
							$term = wp_insert_term(
								...
								array(
									'description'=> '',
									'slug' => strtolower(preg_replace('#[a-zA-Z0-9\-]+#', '', $tax_value)),
									'parent'=> $parent_id
								)
							);
						endif;	
